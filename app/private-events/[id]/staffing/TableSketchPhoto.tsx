    ...
    <div className="flex flex-col gap-3 rounded-lg border border-border-classic bg-surface p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          >
            {isPending ? "מעלה..." : sketchUrl ? "החלף סקיצה" : "העלה סקיצה"}
          </button>
          {sketchUrl && (
            <button
              type="button"
      ...
      {sketchUrl ? (
        isPdf ? (
          <div className="flex flex-col gap-2">
            <embed src={sketchUrl} type="application/pdf" className="h-[600px] w-full rounded-md border border-border-classic" />
          ...
          <a href={sketchUrl} target="_blank" rel="noopener noreferrer">
            ...
            <img src={sketchUrl} alt="סקיצת פריסת שולחנות" className="max-h-[600px] w-full rounded-md border border-border-classic object-contain" />
          </a>
        )
      ) : (
        <p className="text-sm text-foreground/60">העלו תמונה או PDF של סקיצת הפריסה כדי שיהיה ניתן לראות אותה כאן.</p>
      )}
