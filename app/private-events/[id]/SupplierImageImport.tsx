...
import { addSuppliersFromImport, parseSupplierImage } from "@/app/private-events/actions";
...
export function SupplierImageImport({ eventId }: { eventId: string }) {
  ...
  function updateSupplier(index: number, field: keyof SupplierImportDraft, value: string) {
    setSuppliers((prev) => (prev ? prev.map((s, i) => (i === index ? { ...s, [field]: value || null } : s)) : prev));
  }
  ...
  if (!suppliers) {
    return (
      <form onSubmit={handleUpload} className="flex flex-col gap-3 rounded-md border border-border-classic bg-accent-soft/30 p-3">
        ...
        {suppliers.map((supplier, index) => (
          <li key={index} className="flex flex-col gap-2 sm:flex-row sm:items-end">
            <label className="flex flex-1 flex-col gap-1 text-xs">
              <span>תפקיד</span>
              <input value={supplier.role ?? ""} onChange={(e) => updateSupplier(index, "role", e.target.value)} className={inputClass} />
            </label>
            <label className="flex flex-1 flex-col gap-1 text-xs">
              <span>שם</span>
              <input value={supplier.name} onChange={(e) => updateSupplier(index, "name", e.target.value)} required className={inputClass} />
            </label>
            <label className="flex flex-1 flex-col gap-1 text-xs">
              <span>טלפון</span>
              <input value={supplier.phone ?? ""} onChange={(e) => updateSupplier(index, "phone", e.target.value)} className={inputClass} />
            </label>
            <button
              type="button"
