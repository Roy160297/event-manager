...
export default function GuestCsvImport({ eventId }: { eventId: string }) {
    ...
    return (
      <div className="flex flex-col gap-4 rounded-lg border border-border-classic bg-surface p-4">
        <p className="text-sm text-foreground/60">
          זוהה קידוד: {parsed.detectedEncoding} · נמצאו {totalGuestCount} אורחים ({parsed.rows.length} רשומות) בקובץ. הייבוא
        ...
        <div className="flex gap-2">
          ...
          <button onClick={reset} className="rounded-full border border-border-classic px-4 py-2 text-sm hover:bg-accent-soft">
            ביטול
          </button>
        </div>
      </div>
  ...
  return (
    <form onSubmit={handleUpload} className="flex flex-col gap-3 rounded-lg border border-border-classic bg-surface p-4">
      <p className="text-sm font-medium">ייבוא קובץ אורחים (Excel / CSV)</p>
