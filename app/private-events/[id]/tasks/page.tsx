import { createClient } from "@/lib/supabase/server";
import { actionErrorMessage } from "@/lib/actionError";
import { TrashIcon } from "@/components/icons";
import { SaveDetailsForm } from "@/components/SaveDetailsForm";
import { DateField } from "@/components/DateField";
import { createPrivateTask, deletePrivateTask, updatePrivateTask, updatePrivateTaskStatus } from "./actions";
import {
  TASK_PRIORITY_COLORS,
  TASK_PRIORITY_LABELS,
  TASK_STATUS_COLORS,
  TASK_STATUS_LABELS,
  formatDate,
} from "@/lib/labels";
import type { PrivateEventRow, PrivateEventTaskRow, TaskStatus } from "@/lib/types";

const STATUSES = Object.keys(TASK_STATUS_LABELS) as TaskStatus[];
const PRIORITIES = Object.keys(TASK_PRIORITY_LABELS) as (keyof typeof TASK_PRIORITY_LABELS)[];

export default async function PrivateTasksPage({ params }: { params: Promise<{ id: string }> }) {
  const { id: eventId } = await params;
  const supabase = await createClient();

  const [{ data: tasks }, { data: event }] = await Promise.all([
    supabase
      .from("private_event_tasks")
      .select("*")
      .eq("event_id", eventId)
      .order("status", { ascending: false })
      .order("due_date", { ascending: true, nullsFirst: false })
      .returns<PrivateEventTaskRow[]>(),
    supabase.from("private_events").select("*").eq("id", eventId).returns<PrivateEventRow[]>().single(),
  ]);

  const inputClass = "rounded-md border border-border-classic bg-surface px-2.5 py-1.5 text-sm";

  async function addTask(formData: FormData) {
    "use server";
    try {
      await createPrivateTask(eventId, formData);
    } catch (err) {
      return actionErrorMessage(err);
    }
  }

  return (
    <div className="flex flex-col gap-6">
      <SaveDetailsForm
        action={addTask}
        message="המשימה נוספה בהצלחה"
        clearOnSuccess
        className="flex flex-col gap-2 rounded-lg border border-border-classic bg-surface p-3"
      >
        <p className="text-sm font-medium">משימה חדשה</p>
        <div className="grid gap-2 sm:grid-cols-2">
          <input name="title" placeholder="כותרת" required className={`${inputClass} sm:col-span-2`} />
          <textarea name="description" placeholder="תיאור (לא חובה)" rows={1} className={`${inputClass} sm:col-span-2`} />
          <input name="assignee_name" placeholder="אחראי/ת (לא חובה)" className={inputClass} />
          <select name="priority" defaultValue="normal" className={inputClass}>
            {PRIORITIES.map((priority) => (
              <option key={priority} value={priority}>
                עדיפות {TASK_PRIORITY_LABELS[priority]}
              </option>
            ))}
          </select>
          <div className="flex flex-col gap-0.5 sm:col-span-2">
            <DateField name="due_date" defaultValue={event?.event_date ?? ""} />
            <span className="text-xs text-foreground/50">(אם לא ייבחר תאריך, ברירת המחדל היא תאריך האירוע)</span>
          </div>
        </div>
        <button
          type="submit"
          className="self-start rounded-full bg-accent px-3 py-1.5 text-sm font-medium text-accent-foreground hover:opacity-90"
        >
          הוסף משימה
        </button>
      </SaveDetailsForm>

      {(!tasks || tasks.length === 0) && <p className="text-foreground/60">אין עדיין משימות לאירוע זה.</p>}

      <ul className="flex flex-col gap-2">
        {tasks?.map((task) => {
          async function changeStatus(formData: FormData) {
            "use server";
            try {
              await updatePrivateTaskStatus(eventId, task.id, formData.get("status") as TaskStatus);
            } catch (err) {
              return actionErrorMessage(err);
            }
          }
          async function saveEdit(formData: FormData) {
            "use server";
            try {
              await updatePrivateTask(eventId, task.id, formData);
            } catch (err) {
              return actionErrorMessage(err);
            }
          }
          async function remove() {
            "use server";
            await deletePrivateTask(eventId, task.id);
          }

          return (
            <li key={task.id} className="flex flex-col gap-3 rounded-lg border border-border-classic bg-surface p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex flex-col gap-1">
                  <p className="font-medium">{task.title}</p>
                  {task.status === "done" && task.description && (
                    <p className="text-sm italic text-green-700">{task.description}</p>
                  )}
                  <p className="text-sm text-foreground/60">
                    {task.assignee_name ? `אחראי: ${task.assignee_name}` : "ללא אחראי"}
                    {" · "}
                    יעד: {formatDate(task.due_date)}
                  </p>
                  <span className={`self-start rounded-full px-2 py-0.5 text-xs font-medium ${TASK_PRIORITY_COLORS[task.priority]}`}>
                    עדיפות {TASK_PRIORITY_LABELS[task.priority]}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`rounded-full px-2 py-1 text-xs font-medium ${TASK_STATUS_COLORS[task.status]}`}>
                    {TASK_STATUS_LABELS[task.status]}
                  </span>
                  <SaveDetailsForm action={changeStatus} message="הסטטוס עודכן בהצלחה" closeDetailsOnSave className="flex items-center gap-1">
                    <select name="status" defaultValue={task.status} className="rounded-md border border-border-classic bg-surface px-2 py-1 text-sm">
                      {STATUSES.map((status) => (
                        <option key={status} value={status}>
                          {TASK_STATUS_LABELS[status]}
                        </option>
                      ))}
                    </select>
                    <button type="submit" className="rounded-md border border-border-classic px-2 py-1 text-sm hover:bg-accent-soft">
                      עדכן
                    </button>
                  </SaveDetailsForm>
                  <form action={remove}>
                    <button type="submit" title="מחק משימה" className="rounded-md p-1.5 text-red-600 hover:bg-red-50">
                      <TrashIcon className="h-4 w-4" />
                      <span className="sr-only">מחק</span>
                    </button>
                  </form>
                </div>
              </div>

              <details className="border-t border-border-classic pt-2">
                <summary className="cursor-pointer text-xs font-medium text-accent">ערוך משימה</summary>
                <SaveDetailsForm action={saveEdit} closeDetailsOnSave className="mt-2 grid gap-2 sm:grid-cols-2">
                  <input name="title" defaultValue={task.title} required className={`${inputClass} sm:col-span-2`} />
                  <textarea name="description" defaultValue={task.description ?? ""} rows={1} className={`${inputClass} sm:col-span-2`} />
                  <input name="assignee_name" defaultValue={task.assignee_name ?? ""} className={inputClass} />
                  <select name="priority" defaultValue={task.priority} className={inputClass}>
                    {PRIORITIES.map((priority) => (
                      <option key={priority} value={priority}>
                        עדיפות {TASK_PRIORITY_LABELS[priority]}
                      </option>
                    ))}
                  </select>
                  <div className="sm:col-span-2">
                    <DateField name="due_date" defaultValue={task.due_date ?? ""} />
                  </div>
                  <button
                    type="submit"
                    className="self-start rounded-full border border-accent px-3 py-1.5 text-sm text-accent hover:bg-accent-soft sm:col-span-2"
                  >
                    שמור שינויים
                  </button>
                </SaveDetailsForm>
              </details>
            </li>
          );
        })}
      </ul>
    </div>
  );
}
